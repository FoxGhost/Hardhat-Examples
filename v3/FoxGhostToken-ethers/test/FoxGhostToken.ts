...
describe('FoxGhost Token', function(){

    async function deployToken() {
        const [owner, addr1, addr2] = await ethers.getSigners();
    
        const Token = await ethers.deployContract("FoxGhostToken", [owner]);
    ...
    it('Test Approve: spend more than allowed', async function(){
        ...
        await expect(Token.connect(addr1).transferFrom(owner, addr2, BigInt(1) * decimals)).to.be.revertedWithCustomError(Token, 'ERC20InsufficientAllowance');
    ...
    it('Test 0 address: transfer', async function(){
        ...
        await expect(Token.connect(owner).transfer('0x0000000000000000000000000000000000000000', value)).to.be.revertedWithCustomError(Token,'ERC20InvalidReceiver');
    });
    ...
    it('Test 0 address: transferFrom: to', async function(){
        ...
        await expect(Token.connect(addr1).transferFrom(owner, '0x0000000000000000000000000000000000000000', value)).to.be.revertedWithCustomError(Token, 'ERC20InvalidReceiver');
    });

    it('Test 0 address: approve: from', async function(){
        const { Token, owner, addr1, addr2 } = await deployToken();
        await expect(Token.connect(owner).approve('0x0000000000000000000000000000000000000000', value)).to.be.revertedWithCustomError(Token, 'ERC20InvalidSpender');
    ...
    it('Test Events: Transfer', async function(){
        const { Token, addr1 } = await deployToken();
